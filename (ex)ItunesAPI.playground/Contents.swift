...
func getMethod(completion: @escaping ([Music]?) -> Void) {
    ...
    // URL구조체 만들기
    guard let url = URL(string: "https://itunes.apple.com/search?media=music&term=k-pop") else {
    ...
    // 요청을 가지고 작업세션시작
    // 오래걸리는 작업 -> 그래서 내부에 비동기 처리 되어있음. 그래서 함수를 리턴형으로 설계하면 무조건 nil이 나온다. 즉, 콜백함수로 구현해야함.
